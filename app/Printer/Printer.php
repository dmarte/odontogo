...
abstract class Printer extends FPDF
{
    ...
    public function Footer()
    {
        ...
        $this->Cell(
            0,
            10,
            iconv('UTF-8', 'ISO-8859-1', $this->lang['page']).' '.$this->PageNo().' '.iconv('UTF-8', 'ISO-8859-1',$this->lang['page_of']).' {nb}',
            0,
            0,
            'R'
        );
    ...
    public function Body()
    {
        ...
        if ($this->items) {
            foreach ($this->items as $item) {
                ...
                $this->Cell($width_other, $cHeight, $this->quantityField ? $item['quantity'] : null, 0, 0, 'C', 1);
        ...
        if ($this->totals) {
            foreach ($this->totals as $total) {
                ...
                for ($i = 0; $i < $this->columns - 3; $i++) {
                    $this->Cell($width_other - 10, $cellHeight, '', 0, 0, 'R', 0);
...
//                }
                ...
                $this->SetFont($this->font, style: 'b', size: 11);
                $this->SetFillColor($bgcolor, $bgcolor, $bgcolor);
//                if ($total['colored']) {
//                    $this->SetTextColor(255, 255, 255);
//                    $this->SetFillColor($this->color[0], $this->color[1], $this->color[2]);
        ...
        if (!isset($this->productsEnded)) {
            ...
            $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
                ...
                $this->Cell($width_other, 10,
                    iconv(self::ICONV_CHARSET_INPUT, self::ICONV_CHARSET_OUTPUT_A, $this->quantityField ? mb_strtoupper($this->lang['qty'], self::ICONV_CHARSET_INPUT) : null), 0, 0, 'C', 0);
